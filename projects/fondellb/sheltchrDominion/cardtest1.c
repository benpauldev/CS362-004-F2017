...
int main (int argc, char** argv)
{
    ...
    //printf("\nDiscard total before call to playAdventurer: %i\n",discardTotalPrev);
    //printf("Discard total after call to playAdventurer: %i\n\n",discardTotal);

    MY_ASSERT(discardTotal >= (discardTotalPrev));
    if (discardTotal >= (discardTotalPrev))
    {
        test_count++;
    }
    ...
    for (i = 0; i < state.handCount[whoseTurn(&state)]-1 ; i++)
    {
    ...
    //printf("\nCoin card total before call to playAdventurer: %i\n",coinCardCountPrev);
    //printf("Coin card total after call to playAdventurer: %i\n\n",coinCardCount);
